...
> **유효기간 차등 (#30/#799)**: 학생/동료 초대 = **7일**, 학부모 초대(6자리 코드) = **24시간**. 정책은 차등 유지(보안), 모든 초대 생성·공유·입력 면에 유효기간을 병기해 오인 해소.
...
### 3.4 수기 레슨 (Manual Lesson) — 2026-08-03 재정의

> **정의**: 선생님이 레슨 추가 화면에서 직접 등록한 레슨

| 항목 | 설명 |
|------|------|
| **수강권 귀속** | **항상 귀속** — 활성 수강권 자동 연결, 없으면 체험 1회권 자동 생성 (수강권 필수화) |
| **의미 아님** | "수강권 없는 기록용 레슨" 개념은 폐기됨 |
| **편집 권한** | 전체 편집 가능 (수강권 자동생성 레슨과 동일 — 곡/메모 외 날짜·시간도 선생님 관리) |
| **사용하지 않는 표현** | 기록용 레슨, 무귀속 레슨 |
...
### 5.3 수기 학생 (Manual Student) — 2026-08-03 재정의

> **정의**: 앱 미가입 상태로 선생님이 직접 등록·관리하는 학생

| 항목 | 설명 |
|------|------|
| **관계 상태** | 항상 `active` (상태 전이 미적용) |
| **데이터 경로** | 가입 학생과 **동일** (모든 레슨 수강권 귀속) — 차이는 학생 확인 절차의 생략뿐 |
| **수강권** | 즉시 발급만 가능 (제안-수락은 학생 계정 필요). 정식 발급 강제하지 않음 |
| **스케줄** | 선생님 저장 = 즉시 확정 (확인 카드·챗 협상 생략) |
| **앱 연결 시** | 전화번호 매칭 → 이력·수강권 인계 후 가입 학생 규칙 적용 |
| **표시** | 학생 목록 (grey) 오프라인 아이콘 |
...
| 환불 요청 | Refund Request | 잔여 회차 있는 수강권의 외부 이체 환불 요청. `요청됨` → `완료`｜`반려`. 인앱 결제 아닌 수기 이체 |
| 예상 환불액 | Estimated Refund Amount | 환불 정책 기반 참고용 산정치. 최종 금액은 선생님이 처리 시 직접 입력 |
...
| 휴가 구간 | Vacation Segment | 다구간 휴가의 한 기간. 한 번에 여러 비겹침 기간을 등록(구간별 보상옵션, 사유·학생별 예외는 공통). 겹치는 구간은 거부 |
| 일괄 처리 | Batch Action | 스케줄 리스트 다중선택(꾹 눌러 진입) → 선택 N건 일괄 완료/휴강. 완료는 레슨당 1회씩 정확히 N회 차감, 휴강은 차감 없음 |
...
## 15. 그룹레슨 (Group Lesson) — 2026-07-31

| 한글 | 영문 | 설명 |
|------|------|------|
| 그룹 클래스 | Group Class | 정원·노쇼정책을 가진 클래스 정의. 반/드롭인 2유형 |
| 반 (코호트) | Cohort Class | 고정 멤버 반 — 예약 경쟁 없음, 매주 자동 스케줄. 음악 기본 유형. 사용하지 않는 표현: 정규 그룹, 고정반 |
| 드롭인 | Drop-in | 선착 예약 + 정원 경쟁. 특강·원데이·마스터클래스용 |
| 코호트 멤버 | Cohort Member | 반 고정 로스터. 교사 배정 또는 학생 신청→승인 |
| 대기열 자동승격 | Waitlist Auto-Promotion | 정원 취소 시 대기 1순위 자동 확정 + 즉시 알림 |
| 적용 범위 | Applies To | 수강권 사용처: 1:1 전용 / 그룹 전용 / 공용. 사용하지 않는 표현: 별도 그룹 수강권, 그룹 지갑 |
| 반 공지 | Class Announcement | 반 전체 대상 공지 (기존 선생님 공지의 대상 지정) |

> 상세 (FE-BE 클래스 매핑 포함): `.harness/knowledge/glossary.md` §18.
...
| 날짜 | 기존 용어 | 신규 용어 | 이유 |
|------|----------|----------|------|
| 2026-08-03 | 수기 레슨 = 수강권 없는 기록용 | 수기 레슨 = 선생님 직접 등록 (수강권 항상 귀속) / 수기 학생 = 미가입 + 학생 확인 생략 | 수강권 필수화(Plan B) 이후 실체 정합. §3.4·§5.3 신설 (.harness/knowledge/glossary.md SSOT 동기화) |
| 2026-06-19 | (단건 스와이프만) | 일괄 처리 (Batch Action, 다중선택) | #768 ① 스케줄 다중선택 일괄 완료/휴강 — long-press 진입. 완료는 레슨당 정확히 1회 차감(이중/누락 금지), 휴강은 차감 없음. 기존 스와이프 불변 (.harness/knowledge/glossary.md SSOT 동기화) |
| 2026-06-19 | (단일 기간 휴가만) | 휴가 구간 (Vacation Segment, 다구간) | #768 ② 다구간 휴가 — 여러 비겹침 기간을 한 번에 등록(구간별 보상옵션, 사유·학생별 예외 공통). BE 배치 계약 추가, 겹침 거부. 단일 등록 경로 불변 (.harness/knowledge/glossary.md SSOT 동기화) |
| 2026-06-18 | (전화/문자 more 메뉴 2탭) | 신원 스트립 1탭 + 메뉴 관리/상태변경 그룹핑 | 검토 #28 학생 상세 IA — 전화/문자 상단 승격(StudentContactActions), 보관 맨 아래 분리. UI/IA만 (.harness 동기화) |
| 2026-06-18 | (레슨 요약 공유 진입점 부재) | 레슨 요약 공유 (서버 토큰 → URL 복사) | 검토 #62 레슨 상세에 요약 공유 버튼 — 서버 토큰 발급(BE 기구현) → 학생 요약 랜딩 연결. FE-only (.harness 동기화) |
| 2026-06-18 | (만료임박 갱신 CTA 부재) | 갱신 제안 (이전 수강권 프리필 발급) | 검토 #45 임박→갱신 단절 해소 — 만료임박 카드에 갱신 제안 CTA, 이전 값 프리필 발급(교사 확인 후). 발급 로직 불변 (.harness 동기화) |
