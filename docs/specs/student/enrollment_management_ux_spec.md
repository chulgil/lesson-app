...
│ [B] Triage Banner (신규)                         │
│     ┌──────┬──────┬──────┐                      │
│     │만료임박│ 입금대기(후불) │체험중│ ← 탭 → 해당 필터 적용 │
│     │ 3명  │ 2명  │ 1명  │                      │
│     └──────┴──────┴──────┘                      │
├─────────────────────────────────────────────────┤
│ [C] Search + Count/Sort/Filter                   │
│     🔍 검색바                                    │
│     전체 N명  [tune 필터] [정렬]                  │
├─────────────────────────────────────────────────┤
...
| 2 | 입금대기(후불) | `Subscription.status == active && paymentConfirmed == false && paidAt == null` 학생 수 | `AppColors.paperWarn` | 필터 `unpaid` 적용 |
...
### 3.3 Student Card (간결화 — 2026-05-07 재설계)

> **원칙**: 리스트는 식별 + 상태 한눈에 파악. 상세 행동(레슨 추가, 갱신 제안, D-day 경고)은 **상세 화면에서** 수행.
> 이전 설계(D-day chip + 인라인 CTA)는 리스트 카드에 과다 정보를 노출하여 시각적 노이즈를 유발했음.

**카드 구조**:

```
┌─────────────────────────────────────┐
│ [아바타] 김민지 · 바이올린   [4/8]   │  ← 헤더 + 수강권 미니뱃지
│         화 15:00                    │  ← 스케줄
│         ━━━━━━━━━━━━━━━ 4/8회      │  ← 진행 bar (간결)
└─────────────────────────────────────┘
```

**리스트에서 제거된 요소** (상세 화면으로 이동):
- ~~D-day chip (D-10 만료)~~ → 상세 화면 수강권 섹션에서 확인
- ~~[레슨 추가] 인라인 CTA~~ → 상세 화면 + 캘린더에서 수행
- ~~[갱신 제안] 인라인 CTA~~ → 상세 화면 수강권 섹션에서 수행
...
| 현금흐름 파악 | 불가시 | **명료** (입금대기(후불) 배너) |

---

## 10. 선생님 일괄 작업 (Bulk Teacher Actions, §7.119)

> **⚠️ v2 독립 스펙으로 승격됨**: [bulk_teacher_actions_spec.md](./bulk_teacher_actions_spec.md)
> 아래는 v1 레거시 설계. 회차 매핑, 챗 이벤트 통합, 수신 조건 필터, 백엔드 API는 독립 스펙 참조.
...
```
수강 관리 탭 Masthead [체크리스트 아이콘 (tooltip: 학생 선택)] + [+ 아이콘]
  ↓ 체크리스트 아이콘 탭
...
| 2026-05-07 | §10 일괄 작업을 독립 스펙(bulk_teacher_actions_spec.md)으로 승격 — 회차 매핑 + 챗 이벤트 통합 + 수신 조건 필터 + 백엔드 API | Claude |
| 2026-05-07 | §3.3 리스트 카드 간결화 — D-day chip, 레슨추가/갱신제안 CTA 제거 (상세 화면으로 이동). §2.1 전체/학원/개인 SegmentedButton 제거 (그룹 헤더로 충분). 학생추가 FilledButton 제거 (Masthead + 아이콘으로 충분). 선택모드 진입 버튼(checklist_rounded) Masthead에 복원 | Claude |
