...
**제안 과정:**
1. 템플릿 체크박스로 1~N개 선택 (항상 복수 선택 UI)
2. 추천 템플릿 지정 (선택)
3. 메시지 입력 (선택)
4. [제안 보내기]

#### 3.1.3 옵션 제안 (Multi-Choice)

선생님이 여러 템플릿을 체크박스로 선택하면, 학생이 그 중 하나를 라디오 버튼으로 선택한다.

| 시나리오 | 동작 |
|----------|------|
| 1개만 체크 | 기존 단일 제안과 동일 (라디오 없음) |
| 2개 이상 체크 | 학생에게 옵션 표시, 추천 템플릿 기본 선택 |
...
| **단계적 확장** | 무통장입금 → 간편결제 → 카드 순차 확장 |
| **투명한 정산** | 월별 수익 리포트 제공 |
| **앱은 기록만** | 결제 모듈 미제공, 외부 결제 후 기록 관리 |
