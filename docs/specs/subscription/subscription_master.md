...
| **돈은 앱 밖에서, 기록은 앱 안에서** | 결제는 외부(계좌이체/현금), 앱은 기록만 관리 |
...
| 학원 강사 | 모바일 앱 | 레슨 기록 |
| 학원 대표/매니저 | 웹 (예정) | 학원 설정, 수강권 발행, 정산 리포트 |
...
| 원칙 | 설명 |
|------|------|
| **선생님 자율성** | 결제 방식, 미결제 처리, 환불 정책 등 선생님이 설정 |
| **유연한 결제 단위** | 월정액, 패키지(N회권) 선택 가능 |
| **흐름 분리** | 사용자↔사용자(무통장입금 영구) vs 앱→사용자(미래 PG SaaS) |
| **투명한 정산** | 월별 수익 리포트 제공 (선생님이 자체 통장 기준으로 확인) |
| **앱은 기록만** | 사용자↔사용자 결제는 앱이 일체 개입 안 함, 외부 결제 후 기록만 |

#### 4.1.1 흐름 A / A' — 사용자 ↔ 사용자 결제 (영구 PG 미채택)

**대상**:
- **흐름 A**: 선생님 ↔ 학생/학부모 (1:1 레슨 수강료)
- **흐름 A'**: 학원 ↔ 학생/학부모 (학원 수강료)

| 항목 | 정책 |
|------|------|
| PG 연동 | **영구 미채택** (Toss / Portone / 카카오페이 / 이니시스 등 없음) |
| 결제 수단 | **무통장입금 (계좌이체)** — 외부 은행 앱에서 직접 송금 |
| 확인 방식 | **수동 2단계** — 학생/학부모가 [입금완료] 클릭 → 선생님/학원이 통장 확인 후 [입금확인] |
| 자동화 | 입금 매칭 자동화 없음 — 선생님/학원이 통장 사본/은행 알림으로 수동 대조 |
| 영수증 | 앱 발행 없음 — 필요 시 선생님/학원이 외부에서 발행 |
| 환불 | 외부 송금 + 앱은 상태 기록만 (`refunded`) |
| 학원 모드 | 1:1 레슨과 동일 정책 — 학원 ↔ 학생/학부모 직접 송금, 앱은 기록만 |

**근거**:
- 사제지간/학원-수강생 신뢰 관계 — 결제 중개 시 카드사 수수료(2.5~3%) 부담 갈등 회피
- 선생님/학원 자율성 우선 (계좌 / 정산 / 영수증 모두 선생님·학원 결정)
- 통신판매업 신고 / 전자상거래법 / 에스크로 의무 회피 — 앱이 결제 중개자가 되면 법적 책임 폭증
- **앱은 "기록 도구"이지 "결제 중개자"가 아니다**

**백엔드 영향**: `payments` 라우터는 **수동 워크플로우 API만** 제공. PG SDK / webhook / 카드 토큰화 / 자동 매칭 / 영수증 / 정산 / 에스크로 일체 없음.

#### 4.1.2 흐름 B — 앱 → 사용자 결제 (미래 PG 도입 트리거)

**대상**: Lessonaza 앱이 사용자(선생님 / 학부모 / 학생 / 학원)에게 직접 과금

**도입 트리거 시나리오**:
| 시나리오 | 예시 |
|---------|------|
| 선생님 SaaS 구독 | 월 X 원 — 학생 N명 이상 / 무제한 / 분석 리포트 |
| 학생 프리미엄 기능 | AI 피치 / 녹음 비교 / 클라우드 백업 |
| 학부모 가족 플랜 | 자녀 N명 통합 관리 / 알림 강화 |
| 학원 엔터프라이즈 | 학원 단위 라이선스, 강사 N명 / 학생 N명 / 통계 |

**1-Party SaaS 구조** (3-Party 정산 아님):

```
사용자 ──카드/간편결제──▶ PG ──▶ Lessonaza 앱 (B2C SaaS)
                                  │
                                  ├ 구독 활성화 / 기능 잠금 해제
                                  └ 환불: 정기결제 해지 / 일할 환불
```
...
- 플랫폼 수수료 **없음** (전액이 앱 매출, PG 수수료만 차감)

**미정 항목** (PG 도입 결정 시 답해야 함):
- 결제 모델 (정기결제 vs 단건결제 vs 혼합)
- 무료체험 정책 (N일 무료, 카드 등록 여부, 자동 전환)
- 환불 정책 (N일 무조건 환불 vs 일할 vs 환불불가)
- 인앱결제 의무 (iOS/Android 30% 수수료 강제 vs 외부 결제)
- PG 선택 (Toss / Portone / 카카오페이 / 이니시스) — 정기결제 지원/수수료/UX 비교

**격리 원칙**: 흐름 B 도입 시 흐름 A 와 **물리적 분리** (`AppPayment` / `AppSubscription` / `/billing/*`). 흐름 A 는 영구히 무통장입금 유지.

> **상세**: [payment_architecture.md](./payment_architecture.md) — 현행 정책 + 미래 설계 요건 SSOT.
> 옵시디언 사본: `~/Dev/mybrain/10 Projects/레슨앱/기능/결제-아키텍처.md`

### 4.2 Payment Status Model
...
```
pending (입금 대기)
    ├── [학생: 입금완료] → studentConfirmed (확인 대기)
...
#### 4.2.3 2단계 입금확인 워크플로우

```
1단계: 학생/학부모가 "입금완료" 클릭 → studentConfirmed → 선생님 알림
2단계: 선생님이 계좌 확인 후 "입금확인" → confirmed → 수강권 활성화
```

| 시점 | 수신 | 내용 |
|------|------|------|
| 학생 입금완료 | 선생님 | "김민수님이 입금완료를 표시했습니다" |
| 선생님 확인 | 학생 | "입금이 확인되었습니다" |
| 선생님 반려 | 학생 | "입금 내역을 확인해주세요" |
...
| `parent` | 부모 계정 (자녀프로필 또는 학부모 결제 활성화) |
...
여러 Subscription이 하나의 Payment를 참조할 수 있다.

```
학부모 김OO → 40만원 계좌이체 (20만 x 2명)
    → Payment 1건 (40만원, transfer)
    → 김민수 Subscription (20만원, paymentId: pay_001)
    → 김서연 Subscription (20만원, paymentId: pay_001)
```

### 4.7 Refund Policy (환불 정책)

#### 환불 정책 설정 (LessonPolicy 내)
...
8회권 380,000원 (5% 할인), 3회 사용 후 환불:

| 정책 | 계산 | 환불금액 |
|------|------|----------|
| 100% 환불 | 5회 x (380,000 / 8) | 237,500원 |
| 90% 환불 | 237,500 x 0.9 | 213,750원 |
| 회당 재계산 | 380,000 - (3 x 50,000) | 230,000원 |

### 4.8 정산 구조

앱은 정산 기록만 제공, 실제 정산은 외부에서 처리. 학원 모드의 경우 강사별 레슨 횟수 집계를 통한 정산 리포트 제공 (웹).
...
**4. 환불 정책 (RefundPolicy)** → 섹션 4.7 참조
...
| 조항 | 내용 |
|------|------|
| 결제 비개입 | 회사는 결제 대행 미제공. 수강료는 선생님/학원과 학생 간 직접 처리 |
| 분쟁 면책 | 수강료 분쟁에 대해 회사 책임 없음 |
| 수강권 설정 | 가격, 유효기간, 환불 정책은 선생님/학원이 자율 설정 |
| 탈퇴 시 수강권 | 환불 신청 없이 탈퇴 시 잔여 수강권 자동 소멸 |
| 서비스 종료 | 90일 전 통지, 미사용 수강권 환불 |
...
| 항목 | 상태 | 비고 |
|------|:----:|------|
| Payment 모델 기본 필드 | 구현 완료 | Phase 1 |
| 2단계 입금확인 워크플로우 | 구현 완료 | Phase 1 |
| 수강료 관리 화면 (선생님) | 구현 완료 | Phase 1 |
| 청구서(Invoice) 시스템 | 미구현 | Phase 2 |
| 선생님 결제 설정 화면 | 미구현 | Phase 2 |
| 자동 청구서 발행 (월정액) | 미구현 | Phase 2 |
| 환불 처리 | 미구현 | Phase 2 |
| 간편결제 연동 (카카오페이, 토스) | 미구현 | Phase 3 |
| 정산 상세 리포트 | 미구현 | Phase 3 |
