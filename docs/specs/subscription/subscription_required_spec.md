...
>
> **알려진 엣지 (후속 백로그 — 2026-08-04 갱신)**: ① 갱신 선택 → preview 저장 →
> 발급 화면 이탈 시 preview 레슨이 잔존한다 (`_cancel_preview_lessons` 는 제안
> 거절/취소/만료에만 연결 — 제안이 아예 생성되지 않은 경로 미커버. FE 는
> `pushReplacement` 라 add-lesson 화면이 이탈을 관찰할 수 없음 → 발급 화면
> 컨트랙트에 previewLessonId 전달 + 종료 훅 or BE TTL 정리 중 택일 필요). ② ~~활성
> 수강권 2+ 시트 dismiss → 무귀속 저장~~ **해소**: 저장 시점 가드
> (`mustPickSubscriptionBeforeSave`) 가 활성 수강권 보유 + 미선택 저장을 차단하고
> 선택 시트를 재오픈한다 (활성 0개 체험 자동생성·정규 반복은 비대상).
