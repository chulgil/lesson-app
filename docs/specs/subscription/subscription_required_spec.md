...
> **구현 정합 주석 (2026-08-03, code critic 대조)**: 현 구현은 S3 시트의 "갱신 제안
> 보내기"와 "[갱신 발급]"을 모두 발급 화면(Template-First) 라우팅으로 처리한다 —
> 제안 자동 발송 대신 발급 화면에서 제안/즉시 발급을 선택하는 방식(제안 발송 UI 의
> 중복 구현 방지). 이때도 레슨 선생성 없음 원칙은 유지된다(갱신 경로는 preview 로만
> 저장). 위 표의 "복귀 없이 폼 종료 + 안내" 문구 자동화는 미구현 — 후속 백로그.
>
> **알려진 엣지 (2026-08-04 전건 해소)**: ① ~~갱신 선택 → preview 저장 → 발급
> 화면 이탈 시 preview 잔존~~ **해소**: 발급 화면 컨트랙트에 `previewLessonId`
> 쿼리를 전달하고(`renewalIssueLocation`), 발급 없이 화면을 이탈(pop)하면 발급
> 화면이 preview 레슨을 삭제한다 — 발급 성공 시에는 수강권 라이프사이클로 이관
> (`markPreviewLessonHandled`). BE TTL 정리는 채택하지 않음(며칠 뒤 정상 발급될
> preview 오삭제 위험 — "갱신 의도 유효기간" 정책이 불필요한 FE 관찰 방식 선택).
> 잔여 극단 엣지: 발급 화면에서 앱 강제종료 시 preview 잔존 (희귀×희귀 — 재진입
> 갱신 저장 시 새 preview 가 생성될 뿐 회계 영향 없음). ② ~~활성 수강권 2+ 시트
> dismiss → 무귀속 저장~~ **해소**: 저장 시점 가드(`mustPickSubscriptionBeforeSave`)
> 가 활성 수강권 보유 + 미선택 저장을 차단하고 선택 시트를 재오픈한다 (활성 0개
> 체험 자동생성·정규 반복은 비대상).
