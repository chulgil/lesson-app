...
>
> **알려진 엣지 2건 (후속 백로그)**: ① 갱신 선택 → preview 저장 → 발급 화면 이탈
> 시 preview 레슨이 잔존한다 (`_cancel_preview_lessons` 는 제안 거절/취소/만료에만
> 연결 — 제안이 아예 생성되지 않은 경로 미커버). ② 활성 수강권 2+ 학생이 선택
> 시트를 dismiss 하면 subscription_id 없이 저장되어 BE 레거시 무언 보너스 확장에
> 도달할 수 있다 (S3 시트 미도달).
