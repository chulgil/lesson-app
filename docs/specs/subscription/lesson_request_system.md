...
| 선생님 앱: 복수 선택 제안 | ✅ | v4 - 체크박스 선택 → 일괄 수강권 제안 |
...
|------|------|
| 선택 모드 진입 | 헤더 [선택] 버튼 탭 |
| 전체 선택/해제 | 헤더 [전체 선택]/[전체 해제] 토글 |
| 개별 선택 | 카드 탭 또는 체크박스 탭 |
| 선택 모드 종료 | ✕ 버튼 (왼쪽 상단) |
| 일괄 수강권 제안 | 하단 버튼 → `studentIds` 쿼리 파라미터로 전달 |
| 일괄 보류 | 하단 버튼 → 동일 메시지로 일괄 처리 |

**URL 형식:**
```
# 단일 제안
/subscriptions/issue?studentId=student_1

# 복수 제안 (v4)
/subscriptions/issue?studentIds=student_1,student_2,student_3
```

**선택 조건:**
- 대기 중(pending) 상태만 선택 가능
- 만료된 요청은 선택 불가
- 이미 처리된(제안됨, 수락됨, 거절됨) 요청은 선택 불가
