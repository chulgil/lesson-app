...
원칙 (수기 관리 SSOT):
- **PG / 간편결제 / 자동 결제 X** — 학원↔학생 수강료는 **외부**에서 결제 (무통장입금/현금/카드 단말기). 앱은 학원장이 결제 결과를 **수기 마킹**만. 정책 SSOT: [payment_architecture.md](../../subscription/payment_architecture.md) 흐름 A' (학원↔학생/학부모).
- **카드 결제도 외부** — `PaymentMethod.card` = 외부 카드 단말기/POS 에서 학부모 결제 후 학원장이 수기 마킹. 앱이 직접 카드 처리 X.
- **자동 송금 X** — 강사 송금은 학원장이 외부 은행에서 처리 (계좌정보는 학원장 보유). 앱은 `transferred_at` 마킹만.
