...
- 2026-06-05 (3): §7.2 동시 세션 revocation 구현. `create_access_token` 에 jti 자동 부여 (refresh 와 동일 패턴, `uuid4`). `get_current_user` 에 `TokenBlacklist.jti` 체크 추가 — 매칭 시 401, jti 없는 레거시 토큰은 통과 (호환). `switch_context` service 가 `current_jti` 인자를 받아 토글 직후 이전 토큰 jti 를 `TokenBlacklist` 에 add + flush (idempotent 체크 포함, 만료는 access token TTL). endpoint 의 `_extract_context_from_token` 헬퍼를 4튜플로 확장 — jti 도 함께 추출. 회귀 테스트 4건 (이전 JWT → 401 + 새 JWT → 200 / jti 가 blacklist 에 기록 / 레거시 토큰 통과 / 새 토큰 jti 자동 부여 검증). CT-08, CT-09 시나리오 커버. 미해결: 다중 디바이스 일괄 만료, 4시간 후 active_context 자동 복원 (§8.4).
