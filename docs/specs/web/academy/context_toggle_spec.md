...
- 2026-06-05 (5): §7.2 다중 디바이스 일괄 만료 강화. `User.tokens_revoked_at` (nullable datetime) 컬럼 + Alembic migration (`ac_m2_user_tokens_revoked_at`, revises `ac_m2_context_denial_log`). 토글 시 epoch 을 새 토큰 iat 직전(`new_iat - 1 s`)으로 설정 — `get_current_user` 가 `token.iat <= epoch` 시 401. JTI 추적 없이도 같은 user 의 **모든 활성 디바이스**가 한 번에 만료. jti blacklist 는 호출자 토큰 즉시 무효화용으로 유지 (이중 안전장치). 회귀 테스트 3건 (다른 디바이스 토큰 토글 후 401 + 새 토큰 200 / `tokens_revoked_at` 갱신 / epoch 이전 토큰 직접 차단). 본 변경으로 §7.2 단일 + 다중 세션 격리 BE 구현 완료.
