...
> #301 주N회 — 제안-확정 경로는 주1회 유지: 자동 확인카드는 `LessonRequest.preferred_slots`
> 를 주N회로 쓰지 않는다. preferred_slots 는 `WeeklyCalendarPicker` 의 **1·2·3지망 대안**
> (priority)이라, 1순위 슬롯(`preferred_day/time`)으로만 주1회 × totalLessons 생성한다.
> (#890 이 이를 다중슬롯으로 오인해 과다생성 → revert.)
>
> #301 standalone — 주N회는 교사용 경로가 담당: `register_regular_lesson_screen` →
> `POST /bookings`(`fixed_time_slots`)가 N개 동시 주간 슬롯을 운반하면 `ScheduleService.
> create_booking` 이 공유 헬퍼 `_generate_recurring_lessons`(생성기에서 추출)로 위임해
> 분배 생성한다. 회차는 구독 연동 시 `total_lessons`, 없으면 `lessons_per_week × 4`(1개월).
> FE 요일은 1=Mon..7=Sun → BE 0=Mon..6=Sun 변환. 마이그레이션 0.
>
...
> 수·교사·학생·구독). 카드 확정·standalone 양 경로 공통.
