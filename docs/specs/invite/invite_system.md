...
| 생성자 | 역할 | 코드 | 사용자 ID | 비고 |
|--------|------|------|-----------|------|
| 김선생님 | 선생님 | `123456` | teacher_1 | 이미 student_1과 연결됨 |
| 박학생 | 학생 | `654321` | student_1 | 이미 teacher_1과 연결됨 |
| **이선생님** | 선생님 | `TEACH2` | teacher_2 | **테스트용 (미연결)** |
| **이학생** | 학생 | `STUD02` | student_2 | **테스트용 (미연결)** |

### 테스트 시나리오

1. **선생님(teacher_1)으로 테스트**
   - Debug 전환기에서 "선생님" 선택
   - 코드 입력에서 `STUD02` 입력 → 이학생(student_2)과 연결 요청

2. **학생(student_1)으로 테스트**
   - Debug 전환기에서 "학생" 선택
   - 코드 입력에서 `TEACH2` 입력 → 이선생님(teacher_2)과 연결 요청

> **주의**: `123456`, `654321`은 이미 연결된 사용자의 코드라서 "이미 연결되어 있습니다" 오류 발생
> 동일 역할끼리는 연결 불가 (선생님→선생님 코드 사용 시 "연결할 수 없습니다" 표시)
