...
> **흐름 변경 (2026-06-01 E2E 감사 #10 A-C2, 2026-06-02 역할별 차등 보강)**: 선생님은 휴대폰 인증을 SSO 직후에서 분리. 첫 수강권 발급 직전(E3) 게이트로 이동. 학생 직접 가입은 만 14세 검증(정보통신망법 제31조)을 위해 SSO 직후 전화인증 유지. 상세: [phone_verification_policy.md](phone_verification_policy.md)
...
#### 가입 플로우 (선생님 — 2026-06-02)

```
소셜 로그인 + 약관 통합 동의 -> 역할 선택("선생님") -> 이름·악기 1개 -> 홈
  (필수 묶음 1탭 + 마케팅 별도 1탭)                                       ↓
                                                          첫 가용시간 설정 (블로커 퀘스트)
                                                                          ↓
                                                          학생 초대 → 수강권 제안
...
#### Step 1: SSO + 약관 동의 (체크박스 통합)

- 소셜 로그인 (Google/Kakao/Apple)
- 같은 화면에서 약관 동의 체크박스 — 별도 페이지 분리 제거
  - 필수 묶음 1탭: 서비스 이용약관 + 개인정보 처리방침
  - 마케팅 정보 수신 동의: 별도 체크박스 (정보통신망법 제50조 준수)

#### Step 2: 역할 선택

- "선생님으로 시작하기" / "학생으로 시작하기" / "학부모로 시작하기"
- 최초 로그인 시에만 표시
- 학생 선택 시: 전화인증 화면으로 분기 (만 14세 검증 — [phone_verification_policy.md §3.2](phone_verification_policy.md) 참조)
...
|------|:----:|
| 소셜 로그인 + 역할 선택 | ✅ 구현 완료 |
| 휴대폰 인증 | ✅ 구현 완료 (2026-06-02) — 선생님 E3 게이트 + 학생 직접 가입 차단 안전망 + Phase C 퀘스트 wiring |
| 필수 프로필 입력 | ✅ 부분 구현 |
| 튜토리얼 | ❌ 미구현 |
| 프로필 완성도 표시 | ❌ 미구현 |
| 자격증 인증 | ❌ 미구현 |
