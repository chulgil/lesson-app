...
```
소셜 로그인 -> 역할 선택("선생님") -> 휴대폰 인증 -> 필수 프로필 -> 튜토리얼 -> 대시보드
```
...
| 클래스 유형 | 결제 유형 | 결제 주체 |
|------------|----------|----------|
| academy | organization | 학원에서 일괄 결제 -> 선생님은 급여 |
| private | parent | 학부모가 선생님에게 직접 결제 |
...
|
|-- OO음악학원 (3명)          [설정]
|   |-- 김민수  피아노 중급  화 15:00  정규  월 200,000원
|   |-- 이서연  피아노 초급  화 16:00  정규  월 180,000원
|   +-- 박지훈  피아노 입문  수 14:00  체험  월 160,000원
|
|-- (접힘) XX학원 (2명)
|
+-- 개인레슨 (2명)             [설정]
    +-- 강서윤  피아노 고급  금 17:00  학부모: 010-1234-5678
```
