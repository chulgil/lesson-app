...
### 시간 선택 UI 스펙 (네이버 예약 스타일)

> 참조: 네이버 미용실 예약 시스템 UI

#### 1. 기본 원칙

| 항목 | 설명 |
|------|------|
| 시간 간격 | **30분 단위** (정시와 30분: 10:00, 10:30, 11:00...) |
| 섹션 구분 | **오전/오후** 분리 (12시 기준) |
| 레이아웃 | **4열 그리드** |
| 시간 표시 | PM은 **12시간제** (1:00, 2:00 not 13:00, 14:00) |
...
enum TimeSlotStatus {
  ...
  unavailable,  // 선택 불가 (회색 배경, 가용시간 외 또는 레슨 시간 초과)
  booked,       // 예약됨 (회색 배경, 다른 학생 예약)
...
│  오전                                    │
│  ┌────┐ ┌────┐ ┌────┐ ┌────┐          │
│  │9:00│ │9:30│ │10:00│ │10:30│         │  ← 회색: 가용시간 전
│  └────┘ └────┘ └────┘ └────┘          │
│  ┌────┐ ┌────┐                         │
│  │11:00│ │11:30│                        │
│  └────┘ └────┘                         │
...
│  오후                                    │
│  ┌────┐ ┌────┐ ┌────┐ ┌────┐          │
│  │12:00│ │12:30│ │1:00│● │1:30│         │  ← 1:00 선택됨 (Primary)
│  └────┘ └────┘ └────┘ └────┘          │
│  ┌────┐ ┌────┐ ┌────┐ ┌────┐          │
│  │2:00│ │2:30│ │3:00│ │3:30│           │
│  └────┘ └────┘ └────┘ └────┘          │
│  ┌────┐ ┌────┐                         │
│  │4:00│ │4:30│                          │  ← 회색: 레슨시간 초과
│  └────┘ └────┘                         │
└─────────────────────────────────────────┘
```
...
| 화면 | 적용 |
|------|------|
| `TrialLessonRequestScreen` | ✅ 적용됨 |
| `RegisterRegularLessonScreen` | ✅ 적용됨 |
| 1회 레슨 예약 화면 (예정) | ⏳ 적용 예정 |
...
TimeSlotSelector(
  ...
  onTimeSelected: (TimeOfDay) => void,
  availableStart: TimeOfDay,       // 선생님 가용 시작
  availableEnd: TimeOfDay,         // 선생님 가용 종료
  lessonDurationMinutes: int,      // 레슨 시간 (분)
  bookedSlots: List<TimeOfDay>,    // 이미 예약된 시간
  showAmSection: bool,             // 오전 섹션 표시
  showPmSection: bool,             // 오후 섹션 표시
  displayStart: TimeOfDay,         // 표시 범위 시작 (기본 9:00)
  displayEnd: TimeOfDay,           // 표시 범위 종료 (기본 22:00)
)
```
