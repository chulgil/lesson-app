...
| 액션 | 조건 | 이동 대상 |
|------|------|----------|
| 완료 처리 | 항상 표시 | 레슨 상태 변경 |
| **스케줄 변경** | `subscriptionId != null` AND `isUpcoming` | 수강권 상세(챗) |
| **일정 수정** | `subscriptionId == null` (수기 레슨) | 레슨 편집 화면 |
| 취소 | 항상 표시 | 레슨 취소 |

> **지난 레슨**: 스케줄 변경 미표시 (이미 완료/취소된 레슨은 조정 불필요)
> **수기 레슨**: 스케줄 변경 미표시 (편집 화면으로 직접 수정)
