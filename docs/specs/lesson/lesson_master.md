...
##### 취소 마감 판정 주체 (2026-08-05, #1241)

**타이밍 판정의 최종 권위는 서버다.** `LessonPolicy.cancellation_deadline_hours`
(수강권 `override_cancel_deadline_hours` 가 우선)를 기준으로 서버가
`cancelledByStudentAdvance` ↔ `cancelledByStudentLate` 를 확정하며, 클라이언트가
보낸 값이 타이밍과 어긋나면 **양방향으로 정정**한다. 이전에는 선생님이 다이얼로그에서
고른 값이 그대로 저장돼 마감 설정이 아무 효과가 없었다.

| 항목 | 규칙 |
|---|---|
| 판정 대상 | 학생 취소의 사전/지각 구분만 (순수 타이밍 문제) |
| 판정 제외 | 노쇼·선생님 취소·상호 합의 — "누가/왜" 는 선생님 판단 |
| 경계 | 마감 **시각까지는 사전 취소** (경계는 사용자에게 유리하게) |
| 정책 미설정 | 판정하지 않음 — 서버가 없는 페널티를 만들지 않는다 |
| 시간 해석 불가 | 판정하지 않음 (사전 취소 유지) |
| **소급 기록** | 레슨 시작 시각이 지난 뒤의 기록은 **판정하지 않는다** — "지금" 이 학생 통보 시점이 아니므로 증거가 없다. 선생님의 기록이 최종 |

클라이언트는 `GET /lessons/{id}/cancellation-policy` 로 **같은 값을 미리 조회해
힌트로 표시**한다 (마감 경과 시 "사전 취소를 선택해도 지각 취소로 처리됩니다").
힌트는 안내 전용이며 판정은 저장 시 서버가 다시 수행한다. `enforced=false`(정책
미설정)면 배너를 띄우지 않는다 — 지키지 못할 약속 금지.
...
### 5.1.1 피드백 프롬프트 표시 조건 (2026-05-09)

| 조건 | 프롬프트 표시 |
|------|:----------:|
| 수강권 레슨 + 완료 + 피드백 미작성 | **표시** |
| 수기 레슨 (학생 미연결) + 완료 | **미표시** |
| 피드백 이미 작성됨 | 미표시 |

> 수기 레슨은 학생이 앱에 연결되지 않은 경우가 대부분이므로, 피드백 알림을 보여줄 필요 없음.
> 학생에게 전달되지 않는 피드백 작성을 독려하는 것은 불필요한 인지 부하.
...
## 10. 레슨 추가 — 수강권 연동 vs 수기 관리 (2026-05-07, 2026-08-03 Plan B 정합화)

> **2026-08-03 정정**: 아래 원칙은 수강권 필수화(Plan B, [subscription_required_spec.md](../subscription/subscription_required_spec.md))
> 구현 이후의 현재 동작으로 갱신됨. "수강권 없는 기록용 레슨" 개념은 폐기 —
> 모든 레슨은 수강권에 귀속된다. 인텐트 분기·미가입 특칙 SSOT: 위 스펙 §2.6~2.7
> (기획 이력: [lesson_add_intent_redesign.md](../../proposal/lesson_add_intent_redesign.md))

### 10.1 기획 원칙

> **레슨 추가는 수강권 유무와 관계없이 항상 가능하다.**
> 단, 생성된 레슨은 항상 수강권에 귀속된다 — 활성 수강권이 있으면 자동 귀속(다수면 선택 시트),
> 없으면 체험 1회권이 자동 생성·귀속된다 (Plan B §2.1~2.5).

### 10.2 학생 선택 후 안내 배너

수강권 0/1/2+ 분기 배너·선택 시트의 SSOT 는
[subscription_required_spec.md §2.5](../subscription/subscription_required_spec.md)
(구현: `manual_lesson_subscription_section.dart`). 아래 초기(2026-05-07) 문구는 폐기됨.

### 10.3 수기 레슨의 의미 (재정의)

- "수기 레슨" = 선생님이 레슨 추가 화면에서 직접 등록한 레슨 (`LessonSource.manual`) — 수강권 미귀속을 의미하지 않는다
- 수강권 없는 학생에게 등록 시: 체험 1회권 자동 생성·귀속 (기록용 무귀속 레슨 아님)
- 정식 수강권 발급 후: 이후 레슨은 해당 수강권에 귀속 (기존 레슨 소급 재귀속 안 함)
- 미가입(offline) 학생: 동일 경로 사용 — 학생 확인 절차만 생략 (선생님 대리 확정)
...
|------|------|----------|
| 완료 처리 | "완료 처리" | 레슨 상태 변경 |
| 내용 수정 | "내용 수정" | 레슨 편집 화면 (곡/메모만, 날짜·시간 잠금) |
...
> **미가입(수기) 학생 예외 (2026-08-03, §2.7)**: 아래 "수강권 레슨" 잠금은 학생과
> 주고받는 협상(챗)을 전제로 한다. 학생이 앱 미가입(`isAppConnected == false`)이면
> 협상 상대가 없으므로 **수강권 레슨도 수기 레슨과 동일하게 전체 편집을 허용**하고,
> 액션 시트의 "일정 변경(챗)"은 "일정 수정 (직접)"으로 대체된다
> (`subscription_required_spec.md` §2.7 일정 변경 특칙).
...
**수강권 레슨 (미래):**
- 완료 처리
- **내용 수정** → edit_lesson_screen (곡/메모만 편집, 날짜·시간·학생 잠금)
