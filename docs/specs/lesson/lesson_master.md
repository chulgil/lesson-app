...
선생님 발굴 -> 연결 -> 체험 -> 정기 등록 -> 결제 -> 스케줄 -> 레슨 진행 -> 레슨 노트 -> 갱신/종료까지의 흐름을 앱 내에서 처리한다.
...
| **자동화 > 수동** | 리마인더, 횟수 차감, 갱신 안내, 결제 요청은 시스템이 자동 처리 |
...
2. 선생님: 수강권 등록 (현재 잔여 횟수 입력, "결제 완료로 발급" 선택)
3. 선생님: 현재 스케줄 등록
