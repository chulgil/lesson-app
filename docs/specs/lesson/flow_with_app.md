...
sequenceDiagram
    ...
    T->>App: 스케줄 옵션 설정
    Note over App: 🎯 단순화된 제안 UI<br/>⭐ 토요일 15:00 (추천 - 기본 선택)<br/>필요 시 "+ 대안 추가"로 확장
...
### 3.2 선생님 가용시간 설정 및 시간 선택 UI

> **상세 스펙**:
> - [teacher_availability_spec.md](../schedule/teacher_availability_spec.md) 섹션 8 - 시간 선택 UI
> - [Multi_Option_Schedule_Spec.md](Multi_Option_Schedule_Spec.md) Part B 참조

#### 학생용 시간 선택 UI (단순화)

> **설계 원칙**: "생각없이 예약" - 가용 시간만 표시, 원탭 선택
...
```
선생님 가용시간 설정 → 학생 시간 선택 (칩 탭) → 스케줄 확정
```
...
> **상세 스펙**: [subscription_system_spec.md](../subscription/subscription_system_spec.md) 섹션 5.1.1 참조
> - 수강권 발급 UI (템플릿 우선 방식)
> - 결제 상태 옵션
> - 수강권 상태 관리
...
|------|------|
| **핵심** | 템플릿 선택 → 원탭 발급 |
| **개선 효과** | 80% 사용자는 템플릿만 사용, 세부 설정 불필요 |

```
결제 안내 → 학생 결제 (외부) → 선생님 입금 확인 → 템플릿 선택 → 발급
```
...
| 13 | 시간 선택 UI 단순화 | teacher_availability_spec.md | ✅ 섹션 8 |
| 14 | "생각없이 사용" 원칙 | ux_guidelines.md | ✅ 섹션 1.3 |
| 15 | 수강권 템플릿 발급 | subscription_system_spec.md | ✅ 섹션 5.1.1 |
