# Notebook × Score 적용 Phase 로그

> README.md §7 에서 분리 (Wave 2, 2026-04-27).
> 이 문서는 Notebook × Score 디자인 시스템의 **시점별 적용 이력**을 보존한다.
> 신규 화면 추가나 시그니처 적용 시점에는 README 가 정의 SSOT, 이 파일이 phase 기록 SSOT.

## 빠른 내비게이션

### Phase 1~9 — 기반 구축
- [§7.1 Phase 1 산출물](#71-phase-1-실제-산출물)
- [§7.2 검증](#72-검증-결과)
- [§7.3~7.9 Phase 4~9](#73-phase-4-진행-산출물-커밋-a4b8f54f--4094f677--199c491f--6cc52ca0)
- [§7.10~7.11 Phase 3 — CustomPainter 프리미티브](#710-phase-3-산출물-custompainter-프리미티브)
- [§7.12 Gaegu 손글씨 확장](#712-gaegu-손글씨-확장--선생님-피드백--연습-팁--학생-메모)

### §7.13~7.16 — 전역 테마 통일
- [§7.13 바텀 네비](#713-바텀-네비-통일--로마숫자--vermillion-전-역할-확대)
- [§7.14 AppBar Playfair](#714-appbar-타이틀-playfair-통일--전역-테마-1-지점-수정)
- [§7.15 TabBar + Dialog](#715-tabbar--dialog-테마-통일--전역-1-지점으로-4대-시그니처-마감)
- [§7.16 SnackBar](#716-snackbar-테마-통일--앱-전반-알림을-notebook-팔레트로)

### §7.17 — 섹션 헤더 Playfair "정적 명사" 법칙
- [§7.17 섹션 헤더 Playfair 통일](#717-섹션-헤더-playfair-통일--매스트헤드appbar섹션곡명-위계-확립) — 매스트헤드→AppBar→섹션→곡명 위계 확립
- §7.18~§7.29 — Switch/TextSelection·Icon/ListTile·PopupMenu·DatePicker·Radio·Slider 등 위젯별 §7.17 보급
- §7.31~§7.39 — Chip·ExpansionTile·BottomSheet·FilledButton·IconButton·Dialog·FAB 위젯별 §7.17 보급
- §7.40~§7.49 — Dark theme 대칭성 + Material 잔재 ink 치환 + 도메인별 §7.17 배치

### §7.30 — Gothic 예외 ("동적 값" 도메인)
- [§7.30 Gothic 예외](#730-연결-요청-대기-빈-상태-제목-playfair-통일) — 처음 등장 시점은 사실 §7.79 자동 판정 프로토콜에 가까움
- §7.79~§7.85 — search/practice/lessons/subscription `§7.30` 자동 판정 패턴 포화 (3 도메인 0건 변환)

### §7.50~7.65 — Vermillion / paper alpha / shadow 일괄 토큰 치환
- [§7.50 black54·white70 토큰 치환](#750-material-colorsblack54--colorswhite70-8건--notebook-ink--paper-alpha-토큰-치환)
- [§7.53 white.withValues 38건](#753-material-colorswhitewithvaluesalpha-38건--notebook-appcolorspaperwithvaluesalpha-일괄-치환)
- [§7.65 Colors.black 그림자 17 files](#765-colorsblack-그림자--appcolorsink-팔레트-통일-17-files-일괄)

### §7.67~7.78 — students_tab·schedule_tab·domain forms NotebookMasthead 승격
- [§7.67 students_tab masthead](#767-students_tab-헤더-notebook-masthead-승격--탭-레벨-홈-일관성)
- [§7.69 schedule_tab masthead](#769-schedule_tab-헤더-notebook-masthead-승격--탭-레벨-홈-일관성-완결)
- [§7.72 lesson_header_card 히어로](#772-lesson_header_card-히어로-카드-notebook-승격--상세-화면-진입-첫-눈길)

### §7.79~7.86 — 자동 판정 프로토콜 정착 (3-법칙)
- [§7.84 search/practice/lessons §7.30 전수 감사](#784-search-선생님학원-상세-717--practicelessons-730-전수-감사--동적-값-집중-도메인-재확인)
- [§7.86 students/ form 잔재 8 지점 정리](#786-students-form-잔재-750--717-혼합-스윕--vermillion-chipbadgeavatar-8-지점-정리)

### §7.87~7.101 — 학생/선생님 진입 경험 전수 감사
- [§7.87 학생 진입 11 도메인 포화](#787-학생-진입-경험-notebook--score-전수-감사--스케줄student_homeonboarding-717727-포화)
- [§7.91~§7.98 선생님 도메인 Batch B~H](#791-선생님-감사-개시--analytics-730-확정--전체-잔재-로드맵)
- [§7.99~§7.101 비선생님 + 소급 감사](#799-비선생님-도메인-전수-감사--798-전부-730-주장-반증--4-717-추가-승격)

### §7.102~7.109 — 4대 시그니처 SSOT 감사 포화
- [§7.102 Gaegu 전수 감사](#7102--11-4-gaegu-손글씨-서명-최초-전수-감사-2026-04-24)
- [§7.103 Roman numerals 전수 감사 + 경계 4 분류](#7103--11-2-로마숫자-서명-전수-감사-영-후보--경계-케이스-4-분류-2026-04-24)
- [§7.105 Vermillion 전수 감사 + 4대 포화 완결](#7105--11-3-vermillion-red-색-서명-ssot-감사--4대-시그니처-포화-완결-2026-04-24)
- [§7.106 display* 15 지점 §7.30 축 확정](#7106--apptypographydisplay-전수-감사-15-지점-730-전용-축-확정-2026-04-24)
- [§7.107~§7.109 Gaegu 마지널리아 축 포화](#7107--선생님-스케쥴-gaegu-마지널리아-뱃지-교차-화면-승격-2026-04-24)

### §7.110~7.118 — 각진 원칙 (BorderRadius.zero) 확장
- [§7.110~§7.112 스케쥴 리스트/주간 그리드](#7110--스케쥴-리스트-ui-notebook--score-동기화-lessoncard-재사용-2026-04-24)
- [§7.113~§7.114 auth 도메인 각진 포화](#7113--auth-도메인-각진-원칙-확장-로그인-역할-선택-ui-2026-04-24)
- [§7.115 일간 뷰모드 + "오늘" 뱃지](#7115--일간-스케쥴-뷰모드-토글--오늘-뱃지--availability-블록-각진-동기화-2026-04-24)
- [§7.116 student_home 4탭 각진 + 다크 NO-OP 제거](#7116--student_home-4탭-각진-원칙--다크모드-no-op-제거-2026-04-24)
- [§7.117 수강 관리 Status Triage 재설계](#7117--수강-관리-탭-status-triage-재설계-2026-04-24)
- [§7.118 테마 레이어 전수 각진 포화](#7118--테마-레이어-전수-각진-포화-app_themedart--인라인-override-2026-04-24)

### §7.120~7.126 — 평탄화 + 4단 우선순위 + 정렬
- [§7.120 주간 그리드 4단 배경](#7120--주간-그리드-배경-시각-계층-재설계-4단-우선순위-2026-04-24)
- [§7.121 일간 휴무 시각 인코딩](#7121--일간-타임라인-쉬는-날-시각-인코딩-7120-패턴-이식-2026-04-27)
- [§7.122 주간 그리드 4단→2단 단순화](#7122--주간-그리드-컬럼-배경-단순화-4단--2단-2026-04-27)
- [§7.123 휴가·휴무·추가오픈·근무시간 4단 우선순위](#7123--주간-그리드-휴가휴무추가오픈근무시간-4단-우선순위-2026-04-27)
...
### §7.123 — 주간 그리드 휴가·휴무·추가오픈·근무시간 4단 우선순위 (2026-04-27)

**전제**: §7.122 에서 컬럼 변별을 디바이더로 단순화했으나, **`TeacherAvailability.exceptions` (휴가·휴무·추가오픈) 와 `WeeklySchedule.startTime/endTime` (근무시간 경계) 가 시각적으로 미반영**. 사용자 지적: "선생님 설정에 휴무일/휴가/레슨 시작·종료 시간이 있는데 그리드는 정기 휴무만 본다." 진단: §7.122 는 surface request("월화 격색상")만 보고 도메인 모델(`TimeException` 3종 + 근무시간) 전수 조사 없이 구현 → 4가지 케이스 누락.

**문제 재정의**

| 누락 케이스 | 도메인 표현 | §7.122 처리 |
|---|---|---|
| 다일 휴가 | `TimeException(type=vacation)` | 없음 — 정기 근무일이면 평일로 간주 |
| 단일 휴무 | `TimeException(type=holiday)` | 없음 — 정기 근무일이면 평일로 간주 |
| 추가 오픈 슬롯 | `TimeException(type=additionalSlot)` | 없음 — 정기 휴무일이면 그냥 회색 |
| 근무시간 외 셀 | `WeeklySchedule.startTime/endTime` 밖 | 없음 — 근무시간 내와 동일 |

**해결**: 컬럼 단위 4단 우선순위 + 셀 단위 추가 톤 + 라벨 칩으로 분리.

**컬럼 배경 4단 우선순위**

```dart
// schedule_visual_helpers.dart §7.123
Color? weeklyColumnBackground({
  required ScheduleDayType dayType,
  required ColumnRestKind restKind, // none / vacation / holiday / regular
}) {
  if (restKind != ColumnRestKind.none) {
    return AppColors.scheduleRestDayBackground; // #E8E8E8
  }
  if (dayType == ScheduleDayType.today) {
    return AppColors.paperAccent.withValues(alpha: 0.06);
  }
...
| 우선순위 | 상태 | 컬럼 배경 | 라벨 칩 |
|---|---|---|---|
| 1 | 휴가 (다일) | `scheduleRestDayBackground` | "휴가" |
| 2 | 휴무 (단일) | `scheduleRestDayBackground` | "휴무" |
| 3 | 정기 휴무일 | `scheduleRestDayBackground` | (라벨 없음) |
| 4 | 오늘 (정상 근무) | `paperAccent` alpha 0.06 | (라벨 없음) |
| 5 | 과거/미래 평일 | 투명 | — |
...
| 상태 | 셀 톤 | 클릭 |
|---|---|---|
| `additionalSlot` 범위 | 흰색 (휴무 컬럼 위 override) | 활성 |
| 근무시간 외 (정상 컬럼 내) | `ink` alpha 0.03 | 활성 (보강 가능) |
| 근무시간 내 | 투명 | 활성 |

**라벨 칩** (`schedule_weekly_grid_view.dart` 헤더 행 위 오버레이)
- 위치: 시간 그리드 첫 행 위, 컬럼 가로 가운데
- 텍스트: "휴가" / "휴무"
- 스타일: `AppTypography.captionSmall`, `inkSecondary`, `FontWeight.w500`
- 배경: 없음 (컬럼이 이미 회색)

**Lore-directive**: 모든 휴식 종류(휴가·휴무·정기 휴무) 는 단일 색 `scheduleRestDayBackground` (#E8E8E8) 공유 — 색 종류로 변별하지 않고 라벨 칩으로 변별. 이유: 회색 톤을 3종으로 나누면 사용자가 톤 차이를 의미로 학습하기 전에 노이즈로 인지.

**Lore-constraint**: 근무시간 외 셀도 클릭 활성 — 운영 현실상 보강·시험 보충은 정규 근무시간 밖에 잡히는 빈도 높음. 시각만 약화하고 클릭은 허용.

**Lore-constraint**: `additionalSlot` 은 컬럼 라벨 없이 슬롯 자체로 표현 — 휴무 컬럼 위에 흰색 슬롯 하나만 떠있으면 "예외적으로 이 시간만 오픈" 의미 자명.

**Lore-rejected**: 휴가/휴무를 다른 회색 톤(#E0E0E0 / #EEEEEE) 으로 분리 — 톤 차이가 의미로 인지되지 않고 노이즈만 추가. 라벨 칩이 의미 전달 담당.

**Lore-rejected**: 근무시간 외 셀 클릭 비활성화 — 운영 현실 무시. 보강·시험·일회성 레슨은 근무시간 외에 잡히는 케이스 흔함.

**감지 함수** (`schedule_visual_helpers.dart` 신규)

```dart
enum ColumnRestKind { none, vacation, holiday, regular }
...
ColumnRestKind columnRestKindForDate({
  ...
  // 1. vacation (multi-day) 우선
  // 2. holiday (single)
  // 3. regular (weeklySchedules 미등록)
}
...
**테스트 추가**: `schedule_visual_helpers_test.dart` — 케이스별 (휴가 단독, 휴무 단독, 휴가+오늘 충돌, 추가오픈 위 휴무 컬럼, 근무시간 경계 14:00/18:00 inclusive/exclusive).

**호출처**
- `schedule_weekly_grid_view.dart` — 컬럼 배경 + 셀 톤 + 라벨 칩 오버레이
- 칩 위치는 별도 Stack 레이어 (시간 그리드 본문 위)
...
#### §7.126 — 좌측 공간 축소 + 모든 모드 헤더 동일 사이즈 (2026-04-27 같은 날 후속)

**전제**: §7.125 적용 후 사용자 연속 점검 2건:
1. "주간스케쥴의 경우 헤더포함해서 왼쪽으로 더 넓혀야합니다. 현재 왼쪽공간이 많이 남습니다." — 좌측 52px 가 과도. 콘텐츠 영역 확장 필요.
2. (작업 중 정정) "전체,일간,주간 헤더사이즈는 동일하면 좋겠고 주간스케쥴 헤더밑에를 잘 맞추는쪽으로 진행했으면 합니다." — §7.125 의 weeklyGrid 모드 한정 헤더 좌측 +36 분기는 모드 간 헤더 일관성을 깨므로 되돌림. 정렬은 그리드 측 조정으로 수행.

**진단** (§7.125 After 상태)

| 영역 | 좌측 외부 패딩 | 7컬럼 시작 X |
|---|---|---|
| CompactWeekStrip (weeklyGrid) | 52 | 52 |
| CompactWeekStrip (list/timeline) | 16 | 16 |
| WeeklyGrid | 16 + 시간 라벨 36 | 52 |

→ 헤더 시작 X 가 모드별로 달라짐(52 vs 16). 그리드의 좌측 52px(외부 16 + 시간 라벨 36) 가 시각적으로 과도.

**결정 (After §7.126)**

| 영역 | 좌측 외부 패딩 | 좌측 시간 라벨 | 7컬럼 시작 X | 7컬럼 너비 | 우측 외부 패딩 |
|---|---|---|---|---|---|
| CompactWeekStrip (모든 모드) | 16 (screenPadding) | — | 16 | (W − 32) / 7 | 16 |
| WeeklyGrid | 0 | 16 | 16 | (W − 32) / 7 | 16 |

→ 헤더 7컬럼과 그리드 7컬럼 모두 시작 X = 16, 너비 = (W − 32) / 7. 좌측 공간 §7.125 대비 36px 절약.

**구현**:
- `schedule_tab.dart`: §7.125 의 `weeklyGrid` 모드 분기 되돌림. 모든 모드 `padding: fromLTRB(screenPadding, space2, screenPadding, 0)`.
- `schedule_weekly_grid_view.dart`:
  - `const double _weeklyGridTimeColumnWidth = 16` 도입 (시간 라벨 폭).
  - 외부 패딩 비대칭 `EdgeInsets.only(right: screenPadding)` (좌측 0, 우측 16).
  - `cellWidth = (W − screenPadding − _weeklyGridTimeColumnWidth) / 7 = (W − 32) / 7`.
  - 시간 라벨 `SizedBox(width: 16)` + `Padding(right: 1)` + `textAlign: right` — 숫자가 헤더 좌측 빈 패딩 영역(0~16) 에 흡수됨.
  - 셀·now indicator `leftOffset = _weeklyGridTimeColumnWidth + index × cellWidth` (2 곳).

**Why 시간 라벨 폭 = 헤더 좌측 패딩 폭(16)**: 시간 라벨이 헤더의 비어있는 좌측 외부 영역(0~16) 에 시각적으로 흡수됨. 별도 시간 컬럼 영역 없이 정렬과 좌측 공간 절약 동시 달성. captionSmall(10pt) 한 자리 숫자(0~9) + 두 자리 숫자(10~23) 모두 16px 우측 정렬에 안착.

**Why 모든 모드 동일 헤더**: §7.125 의 모드별 분기는 사용자 인지에서 "모드 전환 시 헤더가 움직임" 으로 보임. 헤더는 안정 앵커, 그리드 내부가 적응하는 방향이 일관성 ↑.

**Lore-directive**: 헤더(CompactWeekStrip) 는 모든 모드(list/timeline/weeklyGrid) 동일 사이즈. 정렬은 그리드 측 조정으로 수행 (역방향 분기 금지).

**Lore-directive**: 시간 라벨 폭 = 헤더 좌측 패딩 폭(16). 시간 숫자가 헤더의 비어있는 좌측 외부 영역에 흡수되어 정렬과 좌측 공간 절약 동시 달성.

**Lore-rejected**: 헤더 좌측에 +36 분기 유지 (§7.125) — 모드 간 헤더 일관성 깨짐. 사용자 직접 정정.

**Lore-rejected**: 시간 라벨 폭 24~36 유지 — 좌측 공간 낭비, 헤더 패딩 영역 초과로 정렬 불가능.

**Lore-rejected**: 그리드 좌·우 외부 패딩 모두 0 — 우측 마지막 컬럼이 화면 가장자리에 붙어 시각적 답답함, 헤더 우측(16) 과 정렬 깨짐.
