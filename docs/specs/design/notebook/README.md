...
#### 7.87-i invite/ §7.17 × 1 지점 + §7.30 예외 × 11 지점 확정

**대상**: Batch 3.C — invite/ 도메인 전수 감사 (12 지점 / 6 파일).

**§7.17 BLOCK (1건)**

| 파일 | 라인 | 이전 | 이후 | 근거 |
|------|------|------|------|------|
| invite_confirm_screen.dart | 132 | `AppTypography.headingMedium` (`'$creatorRoleLabel과 연결하기'`) | `NotebookTypography.sectionTitle` | §7.87-h "동적처럼 보이는 정적 명사" — `creatorRoleLabel` ∈ {선생님/학생} enum 유한집합 |

**§7.30 예외 (11건)**

| 파일 | 라인 | §7.30 # | 근거 |
|------|------|---------|------|
| invite_confirm_screen.dart | 84 | #8 상태 헤드라인 | `'연결할 수 없습니다'` 아이콘+메시지 에러 상태 |
| invite_confirm_screen.dart | 393 | #8 상태 헤드라인 | `'이미 연결되어 있습니다'` 중복 연결 에러 |
| invite_confirm_screen.dart | 468 | #8 상태 헤드라인 | `'연결 요청이 전송되었습니다!'` 성공 완료 상태 |
...
**판정 근거**: invite/ 는 **연결 관계 워크플로우** 중심 도메인 — `creatorRole` 로 결정되는 헤드라인 1건만 정적 명사에 해당하고, 나머지는 **에러/성공/빈 상태 헤드라인(§7.30 #7/#8)**, **DB 원천 동적 이름(#2/#6)**, **초대 코드(#10)** 로 모두 §7.30 예외. 전환율 **1/12 ≈ 8%** — 3차 법칙(데이터 비중 높을수록 §7.17 전환율 낮음) 의 하한 샘플.

**검증**: `flutter analyze lib/features/invite/` → No issues found (6.5s).

#### 7.87 시리즈 관찰

**1차 법칙 재확인 (§7.85 의 2차 법칙)**: 한 도메인의 §7.17 승격은 "정적 섹션 명사 소수" 에 집중된다. student_home 22 잔재 중 **11건만 §7.17/§7.27** 이고 나머지 11건은 §7.30 예외 — 즉 **"도메인 전반 확산이 아니라 명사 집중"** (22건 중 50%). onboarding 은 감사 7건 **모두 §7.17 로 확정** (§7.87-g 4건 + §7.87-h 3건 = 7/7, 100%) — 튜토리얼/프로필/인증 스크린이 전부 **정적 카피 중심** 이기 때문이다. **invite 는 1/12 ≈ 8%** — 연결 관계 워크플로우가 **에러/성공 상태 + DB 이름 + 코드** 중심이라 §7.30 예외가 지배적. 네 도메인(스케줄·홈·온보딩·invite) 통합 전환율은 **8% ~ 100% 범위**로, **"도메인의 데이터 비중이 낮을수록 §7.17 전환율이 높다"** 는 3차 법칙이 **invite 하한 샘플로 재확인**.
...
### 7.89 invite/ 다이얼로그 상태 헤드라인 §7.17 확장 — 연결 의례 3종 Playfair 승격

**기간**: 2026-04-23
**배경**: §7.87 학생 진입 감사 Phase 3.C 의 **실제 파일 재검증** 에서 발견. audit 매트릭스 §4 에서 "후보" 로 표기된 5 지점을 파일 Read 로 재분류한 결과, `invite_confirm_screen` 의 **다이얼로그/상태 페이지 헤드라인 3건** 이 정적 카피로 확정 — §7.17 의 "정적 섹션 명사" 정의를 **다이얼로그 상태 헤드라인** 으로 확장 적용.

#### 7.89-a 변경 지점

| 파일 | 라인 | 이전 | 이후 | 상태 |
|------|------|------|------|------|
| invite_confirm_screen.dart | 84 | `AppTypography.headingSmall` (`'연결할 수 없습니다'`) | `NotebookTypography.sectionTitle` | 에러 페이지 |
| invite_confirm_screen.dart | 393 | 동일 (`'이미 연결되어 있습니다'`) | 동일 | 중복 연결 다이얼로그 |
| invite_confirm_screen.dart | 468 | 동일 (`'연결 요청이 전송되었습니다!'`) | 동일 | 성공 다이얼로그 |

**검증**: `flutter analyze lib/features/invite/` → No issues found (7.2s)

#### 7.89-b §7.30 예외 재확정 (6건 — 코드 변경 없음)

| 파일 | 라인 | §7.30 # | 근거 |
|------|------|---------|------|
| my_connections_screen.dart | 309 | #6 아바타 이니셜 | `otherName[0]` 단일 글자 |
| my_connections_screen.dart | 553 | #6 아바타 이니셜 | `name[0]` 단일 글자 |
| invite_confirm_screen.dart | 132 | #2 동적 보간 | `'$creatorRoleLabel과 연결하기'` — role 라벨 런타임 결정 |
| invite_history_screen.dart | 81 | #7 빈 상태 | `'생성한 초대가 없습니다'` 아이콘+메시지 2행 패턴 |
| invite_history_screen.dart | 283 | #10 동적 letterSpacing | `invite.inviteCode` 6자 코드 자간 4 |
| pending_requests_screen.dart | 237 | #6 아바타 이니셜 | `requesterName[0]` 단일 글자 |

**invite/ 도메인 포화도**: 총 13 잔재 → **§7.17 승격 5건** (BLOCK 2 in 기존 배치 + 다이얼로그 확장 3 in §7.89) + **§7.30 예외 8건** = **13/13 확정 (100%)**. 기존 `invite_screen:239` letterSpacing · `my_connections:105` 빈 상태 · `my_connections:315` 동적 이름 · `code_input_screen:221` TextField letterSpacing 을 포함한 §7.30 확정분과 합산.

#### 7.89 시리즈 관찰

**§7.17 "정적 명사" 규칙의 3차 확장**: 원래 정의는 "리스트/탭 내 섹션 헤더" (§7.17). §7.27 에서 "바텀시트 헤더" 확장. §7.89 에서 "**다이얼로그/상태 페이지 헤드라인**" 으로 3차 확장. 공통 규칙: **정적 카피 + 중앙 정렬 + 화면 내 단일 타이틀** = Playfair sectionTitle. 이 3축이 §7.17 의 "정적 명사" 판별 기준.

**"동적처럼 보이는 정적 명사" 패턴의 3번째 반복**: §7.87-h 에서 `data.title` 이 상수 클래스 소스라 §7.17 로 판정된 것처럼, invite_confirm 의 3건은 모두 **AlertDialog/상태 페이지의 조건 분기별 고정 카피**. 조건 분기로 여러 변형이 있지만 각 분기의 **값이 빌드 타임 상수** 이면 §7.17. 분기가 많다고 §7.30 이 되지 않는다 — **출처** 가 동적이어야 §7.30.

**"파일 Read 가 필요한 후보" 의 가치**: §7.88 에서 증명한 "§7.84 10-패턴 자동 판정" 의 정확도 반대편 — **정적 카피/동적 카피의 경계가 모호한 후보** 는 파일 Read 없이 판정 불가. audit 매트릭스에서 "후보" 로 표기된 5건 중 **3건만 실제 §7.17 승격 대상**, 2건은 §7.30 예외로 수렴 (`my_connections:309/553`). 감사 정확도 **60%** — 파일 Read 가 필요한 후보가 전체 잔재의 ~10% 라는 것이 감사 프로세스의 마지막 단계가 된다.

**4차 법칙 — "의례 도메인의 §7.17 전환율"**: §7.82 auth/ 78% · §7.87-g onboarding 100% · §7.89 invite/ 38% (5/13). **의례 도메인(의식적 상태 전환이 있는 도메인)** 은 §7.17 전환율이 *평균 70%* 대로 높게 나타남. gamification/search/auth-input (0%) 대비 압도적. 이는 "데이터 비중 낮음 = §7.17 친화" 법칙의 부분 집합 — 의례 도메인은 **"이 순간 당신에게 일어나는 일의 이름"** 을 명사로 요약하므로 Playfair 에 최적합.

**은유**: §7.89 는 **"결혼식장 입간판 3개를 Playfair 로 바꾸는 일"** — 식장 입구에 "환영합니다" (성공), "예식 준비 중" (중복 연결), "행사 취소" (에러) 세 개의 입간판이 있다. 모두 **정적 카피** 이고, **중앙 정렬** 이며, **의례적 의미** 를 담는다. Gothic 서체로 쓰면 "안내방송" 처럼 들리지만, Playfair 로 쓰면 **"초대장의 연장"** 이 된다. invite 도메인은 그 자체가 **"연결의 의례"** — Playfair 승격이 도메인 정체성과 완벽히 일치.
