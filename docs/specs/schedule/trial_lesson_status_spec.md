...
| **수강권** | 미발급 | 미발급 (레슨 생성 시점에 지연 발급) |

> **실측 정정 (2026-08-12)**: 위 "수강권" 행은 이전에 "확정 시점에 체험 수강권
> 발급됨"으로 기술돼 있었으나, 실제 코드는 예약 확정(`BookingStatus.confirmed`)
> 시점에 수강권을 발급하지 않는다. 체험 1회권(`Subscription.type=trial`)은
> `_create_trial_subscription`(`lesson_service.py`, `LessonService.create` 경유)
> 으로만 생성되며, 이는 선생님이 이후 실제 레슨을 등록할 때(수기 등록 또는
> `UnifiedLessonRequest` 가 정규 전환될 때) 비로소 호출된다 — 확정과 수강권 발급
> 사이에 시차가 있는 "지연 발급" 구조다. 확정 시점 즉시 발급은 원래 의도였고
> 여전히 백로그 옵션으로 남아 있다 — 필요하면 예약 승인 경로
> (`ScheduleService.approve_booking`, `schedule_service.py`)에
> `_create_trial_subscription` 호출을 추가해 구현할 수 있다.
