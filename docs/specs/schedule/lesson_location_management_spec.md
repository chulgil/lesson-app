...
### 6.2 레슨 타입별 장소 옵션 (2026-05-07)

| 레슨 타입 | 선택 가능 장소 | 이유 |
|----------|--------------|------|
| **학원 레슨** (`isAcademy=true`) | 학원, 온라인 | 학원에서 진행하므로 학생집/외부 불필요 |
| **개인 레슨** (`isAcademy=false`) | 학생 집, 외부 스튜디오, 선생님 레슨실, 온라인 | 방문/대면 레슨 장소 다양 |

### 6.3 자동입력 규칙

| 장소 유형 | 주소 소스 | 이동시간 기본값 |
|-----------|----------|---------------|
| 학생 집 (개인) | `Student.address` | 수기 (API 제안) |
| 학원 (학원) | `LessonClass.address` | 0분 (수기 변경 가능) |
| 외부 스튜디오 (개인) | 수기 입력 | 수기 |
| 선생님 레슨실 (개인) | `TeacherProfile.address` | 0분 (고정) |
| 온라인 (공통) | N/A | 0분 (고정) |
...
**핵심 원칙: API 제안 = 참고용, 선생님 입력값 = 확정값**

- API 제안은 입력 편의를 위한 **자동기입**일 뿐, 시스템이 자동으로 확정하지 않음
- `travelTimeMinutes`에 저장되는 값은 **항상 선생님이 최종 확인/수정한 값**
- API 실패 시 **에러 메시지 없음** — 입력란이 빈 상태로 남고 선생님이 직접 입력
- 성공 시 입력박스에 제안값 자동기입 — 선생님이 확인 후 그대로 두거나 수정
- 제안값은 "약 N분" 형태로 5분 단위 올림 (참고 라벨로만 표시)

| 시점 | API 제안 | 선생님 입력 | 확정값 (DB 저장) |
|------|---------|-----------|----------------|
| 수강권 발급 | 약 25분 | 30분 (수정) | **30분** |
| 장소 변경 | 약 15분 | 20분 (수정) | **20분** |
| 직접 수정 | — | 35분 | **35분** |
