...
| **M-3** | §2.2 결정(A-1/A-2) 확정 후: 하단 액션바 위젯 공유(`ScheduleChangeActionBar`) — A-2 채택 시 화면 자체 라우팅 통합까지 | M-2 완료(풀스크린 잔재가 없어야 위젯 공유 시 네비게이션 분기가 단순해짐), §2.2 결정 완료 | A/B 각 Phase 상태별(Default/Waiting/CanRespond/CancellationConfirmed) 스냅샷 테스트 |
| **M-4** | **완료**(브랜치 `feat/m4-grid-mapper`, 머지 대기) — §3.2 그리드 인터랙션 문법 계약을 architecture test로 고정. `PreferredTimeSlot`↔`TimeSlot`↔`TimeSlotOption` 단일 mapper(`lib/features/schedule/domain/mappers/time_slot_mapper.dart`, `toTimeSlotOption()`/`toPreferredTimeSlot()`/`toTimeSlot()`) 도입 — 화면 코드 8곳(`request_detail_screen.dart` 4곳, `request_history_chat.dart` 1곳, `subscription_detail_screen.dart` 3곳)의 인라인 `dayOfWeek ± 1` 변환을 교체. `subscription_detail_screen.dart`의 3곳은 `-1` 보정이 누락돼 있던 실버그(요일이 하루씩 밀려 전송되던 문제)로, 이번 교체로 함께 수정됨 | M-2 (두 그리드 호출부가 안정된 뒤 mapper 위치를 정하는 것이 안전) | `test/features/schedule/domain/mappers/time_slot_mapper_test.dart`(월/일 경계 + 라운드트립) + `test/architecture/schedule_grid_mapper_contract_test.dart`(mapper 외부의 인라인 `dayOfWeek: x.dayOfWeek ± 1` 금지, 두 그리드 `maxSlots=3` 유지) 신규 추가, 전부 통과 |
