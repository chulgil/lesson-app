...
> 트리거 소스 (2026-06-16 확정): "곡/레퍼토리 완성" 진원지는 `features/practice/`의 `repertoire_archive_provider.dart`(`archiveRepertoire()` → `practice_archive_mixin`)다. 별도 `features/repertoire/`·`features/profile/` 아님. P2 와이어링은 여기에 건다.
...
**AC(P1)** — 전부 구현 완료 (머지 `130a6ce3`, 이슈 #733-736 CLOSED)
- [x] 연습 1회 → 해당 날짜 도장 1개(자동), "도장 꾹!" 축하 1회
- [x] 같은 날 재연습 시 도장 중복 생성 안 됨(intensity만 갱신)
- [x] 빈 날이 쉼표로 표기되고 스트릭 압박 문구가 없음
- [x] 부모가 주 1회 확인 도장을 찍고 자녀 장부에 반영됨
- [x] 선생님이 검인 + 빨간펜 한 줄을 남기고 장부에 반영됨
- [x] 2자 fallback(성인/자가학습)에서 빈 노드가 압박 없이 처리됨
- [x] 연령 톤 전환(도장판/연습장)이 자녀 프로필 연령으로 결정됨
- [x] 선생님 미연결 시 학생이 자가 검인(도장 + 한 줄 회고)으로 스스로 평가할 수 있음
- [x] 선생님 연결 시 검인이 과제(assignment) 범위로 한정됨 (자유 연습 전체 평가 안 됨)
- [x] 부모 응원·확인이 평가(점수/판단)가 아닌 정서적 인정으로만 동작함
...
| 곡 완성 트리거 진원지 | **확정(2026-06-16)**: `features/practice/`의 `archiveRepertoire()` (`repertoire_archive_provider.dart`) | §9.2 |
