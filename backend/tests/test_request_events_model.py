...
Validates that 31 RequestEventType values + 2 ScheduleChangeType values
...
@pytest.mark.asyncio
async def test_request_event_persists_31_event_types(db_session: AsyncSession) -> None:
    """RequestEvent model must persist all 31 event types (P0-2).
    ...
    assert len(rows) == 31, f"기대 31 event, 실제 {len(rows)}"
