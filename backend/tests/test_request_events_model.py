...
Validates that 33 RequestEventType values + 2 ScheduleChangeType values
...
@pytest.mark.asyncio
async def test_request_event_persists_33_event_types(db_session: AsyncSession) -> None:
    """RequestEvent model must persist all 33 event types (P0-2).

    Mirrors frontend SSOT in
    `frontend/lib/features/schedule/domain/entities/request_event.dart`
    (RequestEventType, Hive typeId 130, 33 values — scheduleChangeExpired + scheduleChangeReminder
    added in #692).
    """
    ...
    assert len(rows) == 33, f"기대 33 event, 실제 {len(rows)}"
    ...
    assert RequestEventType.lesson_cancelled_by_teacher in {r.event_type for r in rows}
    assert RequestEventType.teacher_announcement in {r.event_type for r in rows}
