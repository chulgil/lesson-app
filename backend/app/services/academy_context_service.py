...
class AcademyContextService:
    ...
    async def switch_context(
        ...
        *,
        ...
        #    활성 토큰도 일괄 만료 (다중 디바이스 동시 세션 금지). epoch 은
        #    새 토큰 iat 보다 1 초 이전으로 설정해 JWT iat 정수 truncation
        #    경계 케이스에서도 새 토큰이 통과하도록 보장.
        ...
        assert new_payload is not None  # 방금 발급한 토큰 — 디코딩 실패 불가
        new_iat = datetime.fromtimestamp(new_payload["iat"], tz=UTC)
        user.tokens_revoked_at = new_iat - timedelta(seconds=1)
        if current_jti:
            existing = await self.db.scalar(select(TokenBlacklist).where(TokenBlacklist.jti == current_jti))
            if existing is None:
                expires_at = new_iat + timedelta(minutes=settings.JWT_ACCESS_TOKEN_EXPIRE_MINUTES)
