...
class RequestEventService:
    ...
    async def get_events_by_request_id(
        self, request_id: str, current_user: Any
        ...
        return [
            RequestEventResponse.model_validate(event) for event in result.all()
        ]

    async def create_event(
        self, request_id: str, data: RequestEventCreate, current_user: Any
        ...
        event = RequestEvent(
            ...
            suggested_slots=(
                [s.model_dump(mode="json") for s in data.suggested_slots]
    ...
    async def _check_request_access(
        self, request_id: str, current_user: Any
    ) -> None:
        ...
        if role == "teacher":
            ...
            teacher_profile_id = await try_resolve_teacher_id(
