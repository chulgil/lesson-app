...
## AC-0 [P2 Visual Growth] 전체 기능 (pending)
...
### AC-1 [도메인 모델] StreakFreeze entity (pending)
- **만족 조건**: AC-1.1 ~ AC-1.3 모두 `passed`
- **담당 job**: Job 1

  #### AC-1.1 [entity] StreakFreeze 5 필드 + 불변 + json round-trip (pending)
...
### AC-2 [성능] GrowthHeatmap 1년 캐시 P95 < 500ms (pending)
- **만족 조건**: AC-2.1 ~ AC-2.3 모두 `passed`. 스펙 §17.
- **담당 job**: Job 2

  #### AC-2.1 [chunk 매핑] chunkIndex(date) 결정적 30일 단위 (pending)
  ...
  #### AC-2.2 [캐시 적중] 13 box read = 1년 데이터 병합 (pending)
  ...
  #### AC-2.3 [invalidation 격리] upsertDay 단일 chunk write (pending)
  - **만족 조건**: `upsertDay` 호출 시 해당 chunk box 만 write. 다른 12 chunk untouched
  - **담당 job**: Job 2 Task 2.1
  - **관련 테스트**: chunk write count assertion

### AC-3 [데이터 레이어] StreakFreezeRepository CRUD (pending)
- **만족 조건**: AC-3.1 ~ AC-3.3 모두 `passed`
- **담당 job**: Job 3

  #### AC-3.1 [인터페이스] 4 메서드 정의 (getOrCreate, grantWeekly, apply, setExamMode) (pending)
  - **만족 조건**: `flutter analyze` 0 issue + 인터페이스 컴파일
  - **담당 job**: Job 3 Task 3.1

  #### AC-3.2 [Mock 구현] 메모리 저장소 4 메서드 검증 (pending)
  - **만족 조건**: 단위 테스트 5/5 통과 (clamp, examMode 분기 포함)
  - **담당 job**: Job 3 Task 3.2

  #### AC-3.3 [Hive 구현] 학생별 단일 record + 빈 box 정상 init (pending)
...
### AC-4 [도메인 서비스] StreakFreezeService 자동 발급/적용 (pending)
- **만족 조건**: AC-4.1 ~ AC-4.4 모두 `passed`. **SC-10 (스펙 §2)** 충족.
- **담당 job**: Job 4

  #### AC-4.1 [자동 발급] Sunday 00:00 KST → +2, max 4 (pending)
...
- **담당 job**: Job 5
  ...
  #### AC-5.2 [D-day 마이그레이션] 1회 실행 + flag idempotent (pending)
  ...
  #### AC-5.3 [안내 토스트] 학생당 1회 표시 + dismiss flag (pending)
  - **만족 조건**: 토스트 노출 후 Hive `streak_freeze_migration_toast_shown` 영속. 재진입 무노출
  - **담당 job**: Job 5 Task 5.2
  - **관련 테스트**: 통합 위젯 테스트

### AC-6 [UI] 1년 히트맵 + 트로피 + 더 보기 화면 (pending)
- **만족 조건**: AC-6.1 ~ AC-6.4 모두 `passed`
- **담당 job**: Job 6, 7, 9

  #### AC-6.1 [YearHeatmapGrid] 365 칸 + 5단계 색 + 색맹 친화 (pending)
...
### AC-7 [SC-11 휴식 권고] 30분 토스트 + 14세 미만 분기 (pending)
- **만족 조건**: AC-7.1 ~ AC-7.3 모두 `passed`. **SC-11 (스펙 §2)** 충족.
- **담당 job**: Job 8

  #### AC-7.1 [세션 30분] 토스트 1회 (반복 0) (pending)
  ...
  #### AC-7.2 [일일 누적 3시간] 별도 차분 토스트 1회 (pending)
  - **만족 조건**: 같은 날 반복 0 (Hive `lastShownDate`)
  - **담당 job**: Job 8 Task 8.1

  #### AC-7.3 [14세 미만 분기] 15분 임계값 강화 (pending)
  - **만족 조건**: Student.birthDate < 14년 → 15분 도달 시 토스트
  - **담당 job**: Job 8 Task 8.1
  - **관련 테스트**: `::test_14_under_threshold`

### AC-8 [E2E + 출시 게이트] (pending)
- **만족 조건**: AC-8.1 ~ AC-8.4 모두 `passed`
- **담당 job**: Job 10

  #### AC-8.1 [E2E A] 스트릭 7일 + 8일째 결석 → freeze 차감 + streak 유지 (pending)
  ...
  #### AC-8.2 [E2E B] 1년 히트맵 + 트로피 + P95 < 500ms (pending)
  - **만족 조건**: integration_test 시나리오 B 통과 + perf 측정 결과 ≤ 500ms (P99 ≤ 1000ms)
  - **담당 job**: Job 10 Task 10.1 + Task 10.2 Step 4
  - **관련 테스트**: `::scenario_b`

  #### AC-8.3 [E2E C] 30분 토스트 1회 + 14세 미만 15분 (pending)
  - **만족 조건**: integration_test 시나리오 C 통과
  - **담당 job**: Job 10 Task 10.1
  - **관련 테스트**: `::scenario_c`

  #### AC-8.4 [회귀 + grep] 전체 flutter test 통과 + SC-4 라벨 0건 (pending)
