...
- [ ] **P1-3 실차감**: 출석 확정 시 기존 `add_usage`(row lock·idempotent) 경로로 실제 차감. pytest: 잔여 감소 + 중복 차감 멱등 + `subscription_deducted` flag-only 경로 제거
  ...
  - **노쇼 BE 집행** (2026-08-18 벤치마크 개정 — 옵시디언 54, 사용자 지시 "유사서비스 패턴 분석 후 개선 기획 적용"): 업계 표준 2축([차감]x[보강권])에 정렬한 3집행 — `deductCredit`(1회 차감) / `reschedule`(1회 차감 + MakeupCredit noShowExempt 1회 적립 30일, 차감 불가 시 적립도 생략 — 쌍 회계) / `noDeduction`(무차감). `halfCredit` 은 **폐기**(시장 부재·정수 원장 충돌·집행 코드 0): 신규 생성·수정 400, enum 값·wire 는 유지(레거시 행은 무차감 처리). 차감은 출석 차감과 같은 `booking.subscription_deducted` 마커 공유(출석↔노쇼 전환 이중 차감 금지), `NoShowRecord(lesson_id=booking.id)` 가 멱등 앵커, 수강권 선택은 J5a `_resolve_deduction_subscription` 재사용(이중 규칙 금지). 노쇼 마킹은 차감 실패(수강권 없음)에도 항상 성공 — 기록만 남김. **노쇼 알림 body 에 정책 결과 문구 포함**(조용한 차감 금지 — 3중 고지) + 배치 출석 경로에도 노쇼 알림 발송(기존 갭 해소). pytest 분기 전부
