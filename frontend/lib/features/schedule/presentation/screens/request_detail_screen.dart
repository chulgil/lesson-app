...
import '../utils/request_detail_phase_filter.dart';
...
import '../widgets/request_detail/request_detail_status_scaffold.dart';
...
class _RequestDetailScreenState extends ConsumerState<RequestDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return requestAsync.when(
      loading: () => const RequestDetailStatusScaffold(
        body: Center(child: CircularProgressIndicator()),
      ),
      error: (error, _) => RequestDetailStatusScaffold(
        body: Center(
          child: Text(
      ...
      data: (request) {
        if (request == null) {
          return RequestDetailStatusScaffold(
            body: Center(
              child: Text(
  ...
  Future<void> _handleCounterPropose(
    ...
    // Open schedule comparison bottom sheet with student's preferred slots
    // (P1-4 first increment — migrated from full-screen push to the house
    // bottom-sheet pattern used by the rest of RequestDetailScreen's actions).
