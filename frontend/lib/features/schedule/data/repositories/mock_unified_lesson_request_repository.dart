...
class MockUnifiedLessonRequestRepository
  ...
  void _seedData() {
    ...
    _addEvents('ulr_12', [
      ...
      RequestEvent(
        ...
        createdAt: today.subtract(const Duration(days: 26, hours: 12)),
      ),
      RequestEvent(
        id: 'evt_12_6',
        requestId: 'ulr_12',
    ...
    _addEvents('ulr_13', [
      ...
      RequestEvent(
        ...
        actorId: 'teacher_1',
        eventType: RequestEventType.paymentNotified,
        message: '입금 확인 요청을 받았습니다',
        createdAt: today.subtract(const Duration(days: 5, hours: 12)),
      ),
      RequestEvent(
        id: 'evt_13_6',
        requestId: 'ulr_13',
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // 16. 수강권 수락 → 입금대기(후불) (Phase 2) — proposalAccepted
    ...
    // Boundary-only request-decision fixtures. They are intentionally attached
    // to a non-primary teacher so the legacy "17 seed requests" teacher_1
    // contract stays stable while getById/getEvents cover the edge cases.
    _addRequest(
      UnifiedLessonRequest(
        id: 'ulr_18',
        studentId: 'student_boundary_1',
        teacherId: 'teacher_boundary',
        type: LessonRequestType.regular,
        instrument: '바이올린',
        goal: UnifiedLessonGoal.hobby,
        experience: UnifiedExperienceLevel.beginner,
    ...
    _addRequest(
      UnifiedLessonRequest(
        id: 'ulr_19',
        studentId: 'student_boundary_2',
        teacherId: 'teacher_boundary',
        type: LessonRequestType.regular,
        instrument: '피아노',
        goal: UnifiedLessonGoal.hobby,
        experience: UnifiedExperienceLevel.intermediate,
        message: '대안 수락 경계 케이스',
        preferredDuration: 60,
        preferredSlots: [
          PreferredTimeSlot(
            priority: 1,
    ...
    _addRequest(
      UnifiedLessonRequest(
        id: 'ulr_21',
        studentId: 'student_boundary_4',
        teacherId: 'teacher_boundary',
        type: LessonRequestType.regular,
        instrument: '플루트',
        goal: UnifiedLessonGoal.hobby,
        experience: UnifiedExperienceLevel.beginner,
        message: '결정 변경 경계 케이스',
        preferredDuration: 60,
        preferredSlots: [
          PreferredTimeSlot(
            priority: 1,
  ...
  Future<UnifiedLessonRequest> withdrawApproval(String id) async {
    ...
    final request = _requests[id];
    if (request == null) throw Exception('Request not found: $id');
    final updated = request.copyWith(status: UnifiedRequestStatus.pending);
    _requests[id] = updated;
    return updated;
  }
  ...
  Future<UnifiedLessonRequest> counterPropose(
    ...
    final request = _requests[id];
    if (request == null) throw Exception('Request not found: $id');
    final updated = request.copyWith(currentRound: request.currentRound + 1);
    _requests[id] = updated;
