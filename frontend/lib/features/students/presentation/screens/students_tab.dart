...
import '../../../onboarding/onboarding_facade.dart'
    show StarterSampleCleanupBanner, StarterSampleOffer;
...
class _StudentsTabState extends ConsumerState<StudentsTab> {
        ...
        RefreshIndicator(
          ...
          child: CustomScrollView(
            ...
            slivers: [
              ...
              // UXB-1 — 실제 학생이 생긴 뒤에만 스스로 나타나는 예시 데이터 정리 CTA.
              if (!_isSelectionMode)
                const SliverToBoxAdapter(child: StarterSampleCleanupBanner()),
        ...
        children: [
          ...
          NotebookMasthead(
            ...
            trailing: Row(
              ...
              children: [
                // 📢 공지 — 탭: 공지 이력 (이력 화면에서 + 버튼으로 새 공지)
                IconButton(
                  onPressed: () => _showAnnouncementHistory(context),
                  icon: const Icon(
                    Icons.campaign,
                    color: AppColors.ink,
                    size: 22,
                  ),
                  tooltip: AppStrings.announcementHistoryTitle,
  ...
  Widget _buildEmptyState() {
    ...
    final emptyState = EmptyStateWidget(
    ...
    // 검색 결과 없음과 명단 자체가 빈 것은 다르다 — 둘러보기 제안은 후자에서만
    // 노출한다 (UXB-1).
    if (query.isNotEmpty) return emptyState;

    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      mainAxisSize: MainAxisSize.min,
      children: [emptyState, const StarterSampleOffer()],
    );
  }
