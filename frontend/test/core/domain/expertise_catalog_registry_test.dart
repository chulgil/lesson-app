...
void main() {
  group('ExpertiseCatalogRegistry', () {
    test('music 카탈로그 = id instruments, 18개 악기', () {
      expect(ExpertiseCatalogRegistry.music.id, 'instruments');
      expect(ExpertiseCatalogRegistry.music.items.length, 18);
  ...
  group('InstrumentList 별칭 계약 (음악 악기 회귀 가드)', () {
    test('InstrumentList.all === music 카탈로그 items (동일 18개·동일 순서)', () {
      expect(InstrumentList.all, ExpertiseCatalogRegistry.music.items);
      expect(InstrumentList.all.length, 18);
    });
