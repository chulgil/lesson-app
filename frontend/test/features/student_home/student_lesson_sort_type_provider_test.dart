...
import 'package:lessonaza/features/student_home/presentation/providers/student_lessons_tab_state_provider.dart';

/// Waits until [provider]'s value satisfies [predicate], or times out.
///
/// [StudentLessonSortType.build] persists via a fire-and-forget Hive load,
...
void main() {
  ...
  ProviderContainer container({String userId = 'student_1'}) {
  ...
  test('첫 실행(persist 이력 없음)에는 시간순 기본값을 유지한다', () async {
    final c = container();
    expect(c.read(studentLessonSortTypeProvider), LessonSortType.timeAsc);
  ...
  test('선택한 정렬이 컨테이너 재생성(앱 재시작 시뮬레이션) 후에도 복원된다', () async {
    final c1 = container();
    await c1
        .read(studentLessonSortTypeProvider.notifier)
        .setSortType(LessonSortType.nameAsc);

    final c2 = container();
    final restored = await _waitForState(
      c2,
      studentLessonSortTypeProvider,
      (type) => type == LessonSortType.nameAsc,
    );
    expect(restored, LessonSortType.nameAsc);
  });

  test('다른 studentId 는 저장된 선택과 독립적으로 기본값을 유지한다', () async {
    final studentA = container(userId: 'student_a');
    await studentA
        .read(studentLessonSortTypeProvider.notifier)
        .setSortType(LessonSortType.nameAsc);

    final studentB = container(userId: 'student_b');
    await Future<void>.delayed(const Duration(milliseconds: 100));
    expect(
      studentB.read(studentLessonSortTypeProvider),
