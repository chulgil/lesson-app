...
- 5-3d-7 urgent_alert_zone + lesson_request_section 3 사이트 (2026-04-30) — home/ 도메인 잔여 위젯 2개 번들 종결. urgent_alert_zone.dart 2건(미수금 alert 텍스트+만원/원 단위 캡슐화 = `urgentAlertOutstandingFormat(int total, int count)` 포매터 / 토글 라벨 `urgentAlertCollapse`+`urgentAlertMoreFormat`) + lesson_request_section.dart 1건(전체보기 → viewAll 재사용). 신규 키 3개 + 재사용 1건. flutter analyze — No issues. 한글 grep — 0 잔존. 커밋 `67cbe3b8` (키) + `95b0b2dc` (마이그레이션). **번호 충돌 처리 (재발)**: 키 커밋(67cbe3b8) 시 외부 세션의 unstaged all_recordings_screen 도메인 키 67줄(외부 세션이 5-3d-5 라벨로 작업 중)이 무차별 번들링됨 → 본 세션 5-3d-7 키 커밋이 외부 세션 5-3d-5 키까지 흡수. 키 자체는 멱등이므로 functional 회귀 없으나 commit attribution 분산. **교훈**: 같은 파일에 양 세션 변경이 누적되면 `git add <file>` 순간 모두 흡수됨 → 협업 모드에서는 `git stash` + `git add -p` hunk 단위 stage 필요. **home/ 도메인 종결**: production presentation 코드 100% 완료 (dashboard_tab 13 + getting_started_card 7 + lesson_card 5 + home_screen 4 + urgent_alert_zone 2 + lesson_request_section 1 = 32 사이트). 잔여 0건 (assignment_dashboard_screen / assignment_summary_section / time_context_banner / teacher_subscription_section / schedule_change_request_section 5파일은 한글 grep 0).
...
- 5-3d-12 invite_screen 26 사이트 (2026-04-30) — invite/ 초대 생성 화면(449줄). AppBar 분기 2건(teacher/student) + history 툴팁 1건 + error 2건(title/subtitle) + targetRole 분기 1건(AppStrings.student/teacher 재사용) + 헤더 안내 포맷터 1건(inviteShareGuideFormat) + QR 카드 2건(qrCodeSectionTitle/qrCodeScanInstruction) + 코드 카드 3건(inviteCodeSectionTitle/inviteCodeCopyTooltip/inviteCodeManualEntryHint) + 공유 버튼 3건(inviteShareLinkButton/inviteCopyLinkButton/inviteKakaoButton) + 만료 포맷터 1건(inviteExpiryFormat) + alternative 헤더 1건 + QR 스캔 ListTile 3건(title/teacher subtitle/student subtitle) + 코드 입력 ListTile 2건(inviteCodeAppBarTitle 재사용/inviteCodeInputShortSubtitle) + 선생님 검색 ListTile 2건(title/subtitle) + SnackBar 2건(code copied/link copied) + share 메시지 포맷터 1건(multi-line: code+url+role) + share subject 1건. 신규 키 24개 + 재사용 3건(student/teacher/inviteCodeAppBarTitle) + 기존 retry 1건. **단독 커밋**: 키 `46bbc07f` + 마이그레이션 `f205da44` — 외부 세션 번들링 회피 성공. flutter analyze — No issues. 한글 grep — 0 잔존. **번호 처리**: 외부 세션이 5-3d-11(code_input_screen) 선점 → 본 세션 5-3d-12 채택. **invite/ 도메인 잔여**: my_connections / invite_confirm / invite_history / scan_invite (외부 세션 grep 카운트는 별개).
- 5-3d-13 scan_invite_screen 11 사이트 (2026-04-30) — invite/ QR 스캔 화면(323줄). 동적 targetRole 분기 1건(student/teacher 재사용) + AppBar 포매터 1건($targetRole QR 스캔) + tooltip 2건(플래시/카메라 전환) + scan 안내 2건(prompt format/frame instruction) + 대안 진입 1건(코드로 입력하기) + 에러 3건(invite invalid 재사용/QR invalid/processing error). 신규 키 8개 (inviteScanAppBarTitleFormat/inviteScanFlashTooltip/inviteScanCameraSwitchTooltip/inviteScanPromptFormat/inviteScanFrameInstruction/inviteScanEnterCodeAlternative/inviteScanInvalidQr/inviteScanProcessingError) + 재사용 3건(student/teacher/inviteCodeInvalid). **단독 커밋**: 키 `cce19225` + 마이그 `fd05a638` — 5-3d-12 invite_screen 의 inviteScanQr* 와 prefix 충돌 없음. flutter analyze — No issues. subscription 63/63 PASS. 한글 grep — dev comment 5건만 (Notebook×Score 정책 참조 보존). **invite/ 도메인 잔여**: my_connections / invite_confirm / invite_history.
- 5-3d-14 invite_history_screen 17 사이트 (2026-04-30) — invite/ 초대 내역 화면(385줄). AppBar 1건(inviteHistoryTooltip 재사용) + error 2건(load error retry/description) + empty 2건(title/body) + 섹션 헤더 2건(active/inactive) + 취소 다이얼로그 4건(title/content/no/confirm — confirm 은 cancelRequestAction 재사용, no 는 기존 '아니오' 표기 유지) + 취소 SnackBar 1건 + 카드 복사 SnackBar 1건(재사용) + 카드 복사 tooltip 1건(재사용) + 사용 횟수 포매터 1건(inviteUseCountFormat) + 카드 액션 버튼 1건(inviteRevokeDialogTitle 공용) + _formatDate 3건(todayLabel/yesterdayLabel/timeAgoDays — todayLabel/timeAgoDays 글로벌 재사용). 신규 키 12개 + 재사용 6건(inviteHistoryTooltip/cancelRequestAction/inviteCodeCopiedSnack/inviteCodeCopyTooltip/todayLabel/timeAgoDays). **단독 커밋**: 키 `f670fbb6` + 마이그 `7a096fb9` — 외부 세션 5-3d-13 라벨 선점에 따라 5-3d-14 채택. flutter analyze — No issues. 한글 grep — 0 잔존 (numeric '${month}/${day}' 는 locale-agnostic 이라 i18n 대상 아님). **invite/ 도메인 잔여**: my_connections / invite_confirm.
...
| P2 5-3d-7 urgent_alert_zone + lesson_request_section i18n (3 사이트) | ✅ 완료 (2026-04-30) |
| P2 5-3d-8 all_recordings_screen i18n (24 사이트) | ✅ 완료 (2026-04-30) |
...
| P2 5-3d-12 invite_screen i18n (26 사이트) | ✅ 완료 (2026-04-30) |
| P2 5-3d-13 scan_invite_screen i18n (11 사이트) | ✅ 완료 (2026-04-30) |
| P2 5-3d-14 invite_history_screen i18n (17 사이트) | ✅ 완료 (2026-04-30) |
| **다음** P2 5-3d-15+ invite/ 도메인 잔여 (invite_confirm / my_connections) | 대기 |
