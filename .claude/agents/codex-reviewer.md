---
name: codex-reviewer
description: Claude 가 작성한 코드를 OpenAI Codex (다른 LLM 가족) 로 독립 검증. 같은 모델 가족 내의 맹점을 교차 모델로 노출. Phase 6 Oracle Problem 완화 옵션.
---
...
**필수**:
- 보안 / 결제 / 마이그레이션 (ultra 모드) 의 Phase 6

**선택**:
- feature (balanced) Phase 6 Code Critic 의 2차 검증
...
git diff --cached | codex exec --model gpt-5-codex \
  "아래 diff 를 리뷰해주세요. 스펙: $(cat .harness/spec/{feature}.md).
   평가 항목: 스펙 정렬, 엣지 케이스, 보안, 경계 검증.
   결과는 200 단어 이내 구조화 포맷."
```

Codex CLI 의 출력을 `.harness/journal/{date}-codex-review-{feature}.md` 에 저장.
...
| Claude Code Critic | Codex Reviewer | 액션 |
|---|---|---|
| PASS | PASS | 진행 |
