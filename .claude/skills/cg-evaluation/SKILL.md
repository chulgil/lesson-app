...
- Phase 5 의 구현 결과
- `.harness/spec/{YYYY-MM-DD}-{feature}.md`

## 출력

- `.harness/journal/{YYYY-MM-DD}-eval-{feature}.md`
...
**왜 이 순서인가**: LLM 기반 Critic 호출은 시간·토큰 비용이 크다. 린트/테스트/빌드
순으로 빠른 실패를 먼저 발견하면 LLM 호출을 아낄 수 있고, 피드백 루프도 짧아진다.
...
## Critic 2.7 — Codex Reviewer (옵션, 교차 모델)

Claude 로 작성된 코드를 **다른 모델 가족 (OpenAI Codex)** 이 독립 검증. 같은 모델 가족의 훈련 데이터 편향을 교차 모델로 노출.

```
# 외부 Codex CLI 호출 (Claude 세션 내 subagent 아님)
git diff --cached | codex exec --model gpt-5-codex \
  "아래 diff 를 리뷰: 스펙 정렬, 엣지 케이스, 보안, 경계 검증.
   스펙: $(cat .harness/spec/{feature}.md)
   결과는 200 단어 이내 구조화 포맷."
```

전제 조건:
- `npm i -g @openai/codex` 설치
- `codex login` 으로 ChatGPT Plus/Pro 계정 OAuth 로그인 (API 키 모드도 가능)
- 출력은 `.harness/journal/{date}-codex-review-{feature}.md` 에 저장

적용 기준:
- **ultra 모드**: 강력 권장 (Oracle Problem 강 완화)
- **balanced**: 선택. Code Critic 결과에 확신이 없을 때
- **fast**: 불필요
...
|---|---|---|
| PASS | PASS | 진행 |
| PASS | FAIL | Codex 이슈 우선 검토 (모델 편향 노출) |
